...
def construct_feature_columns():
    """Construct the TensorFlow Feature Columns.
    构建特征列，这之前的函数是构建模型之前对数据进行处理的主要函数，可以进行
    1、特征的删选和处理
    2、标签的处理
    3、这个函数是说明要把哪些特征传递给模型，在这里可以做独热编码，分箱等
...
def train_linear_classifier_model(
    ...
    my_optimizer = tf.train.FtrlOptimizer(
        learning_rate=learning_rate, l1_regularization_strength=regularization_strength)
...
if __name__ == "__main__":
    ...
    print("Accuracy on the validation set: %0.2f" %
    ...
    validation_probabilities = np.array(
        [item['probabilities'][1] for item in validation_probabilities])
